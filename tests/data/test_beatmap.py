...
note_type = {'use_colorNotes': True, 'use_chains': True, 'use_bombNotes': False, 'use_obstacles': False, 'use_arcs': False}
...
kwargs = {'segment_duration': 30, 'num_samples': 1000000, 'shuffle': True, 'sample_on_duration': False, 'sample_on_weight': False, 'min_segment_ratio': 0.8, 'position_size': 12, 'token_id_size': token_id_size, 'note_type': note_type, 'shuffle_seed': 0, 'permutation_on_files': False, 'merge_text_p': 0.25, 'drop_desc_p': 0.5, 'drop_other_p': 0.5, 'sample_rate': 32000, 'channels': 1}
path = '/home/kk/project/audiocraft2/egs/bs_rank'
...
for file_meta in tqdm(dataset.meta, desc="Processing files"):
    ...
    if note_type['use_chains'] and file_meta.arcs_num == 0:
        continue
    with open(file_meta.beatmap_file_path, 'r', encoding = 'utf-8') as f:
                    beatmap_file = json.load(f)
    beatmap = data.audio_dataset_beatmap.Beatmap(minimum_note = 0.125, duration = file_meta.duration, token_id_size = dataset.token_id_size, position_size = dataset.position_size, **dataset.note_type)
    segment_duration_in_quaver = round(file_meta.duration / 60 * file_meta.bpm *8)
    beatmap_file = beatmap.sample_beatmap_file(beatmap_file, 0,  segment_duration_in_quaver)
    beatmap_token, note_code_map = beatmap.tokenize(beatmap_file, file_meta.bpm, segment_duration_in_quaver)
    reconstructed_beatmap_file = beatmap.detokenize(beatmap_token[note_code_map])
