...
class RotaryEmbeddingCrossAttention(nn.Module):
    ...
    def __init__(self, dim: int, max_period: float = 10000.0, xpos: bool = False,
                 scale: float = 1.0, shared_steps: int = 1, unique_steps: int = 1, device=None, dtype: torch.dtype = torch.float32):
        super().__init__()
        assert dim % 2 == 0
        self.scale = scale
        ...
        self.rotation_k: tp.Optional[torch.Tensor] = None  # For key rotation in cross-attention.

        self.xpos = XPos(dim, device=device, dtype=dtype) if xpos else None

    def get_rotation(self, start: int, end: int, is_k):
        """Create complex rotation tensor, cache values for fast computation."""
        # is_k = True if the rotation is for key, False if for query.
        ...
        if not is_k:
            ...
            if self.rotation is None or end > self.rotation.shape[0]:
                ...
                idx = torch.arange(self.unique_steps, device=self.frequencies.device, dtype=self.dtype)
                angles = torch.outer(idx, self.frequencies)
                self.rotation = torch.polar(torch.ones_like(angles), angles)[int(self.unique_steps/2)].repeat(end, 1)
            return self.rotation[start:end]
        else:
            assert start % self.unique_steps == 0
            assert end % self.unique_steps == 0
            end_unique = end // self.unique_steps
            if self.rotation_k is None or end > self.rotation_k.shape[0]:
                assert isinstance(self.frequencies, torch.Tensor)
                idx = torch.arange(self.unique_steps, device=self.frequencies.device, dtype=self.dtype)
                angles = torch.outer(idx, self.frequencies)
                self.rotation_k = torch.polar(torch.ones_like(angles), angles).repeat(end_unique, 1)
            return self.rotation_k[start:end]
    ...
    def rotate(self, x: torch.Tensor, start: int = 0, time_dim: int = 1, invert_decay: bool = False):
        ...
        target_shape[-1] = -1
        rotation = self.get_rotation(start, start + T, invert_decay).view(target_shape)

        if self.xpos:
            decay = self.xpos.get_decay(start, start + T, invert_decay).view(target_shape)
        else:
            decay = 1.0
    ...
    def rotate_qk(self, query: torch.Tensor, key: torch.Tensor, start: int = 0, time_dim: int = 1):
        ...
        query_out = self.rotate(query, start, time_dim)
        key_out = self.rotate(key, int(start // self.shared_steps * self.unique_steps), time_dim, invert_decay=True)
