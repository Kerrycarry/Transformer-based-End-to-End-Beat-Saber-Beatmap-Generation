...
class StreamingMultiheadAttention(StreamingModule):
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor,
        ...
        if not self.block_self_attention:
            # musicgen case
            if self.causal:
...
class StreamingTransformer(StreamingModule):
    ...
    def __init__(self, d_model: int, num_heads: int, num_layers: int, dim_feedforward: int = 2048,
                 ...
                 checkpointing: str = 'none', position_size: int = 12, block_pos_embeding: bool = False, device=None, dtype=None, **kwargs):
        super().__init__()
        ...
        self.position_size = position_size
        self.block_pos_embeding = block_pos_embeding
        if block_pos_embeding:
