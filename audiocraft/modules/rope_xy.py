...
def compute_axial_cis(dim: int, end_x: int = 4, end_y: int = 3, theta: float = 100.0, normalize=32):
...
def reshape_for_broadcast(freqs_cis: torch.Tensor, x: torch.Tensor):
    ndim = x.ndim
    assert 0 <= 1 < ndim
    # if freqs_cis.shape == (x.shape[-3], x.shape[-1]):#L,C
    ...
    shape = [1, x.shape[-3], 1, x.shape[-1]]
    segment_duration = x.shape[-3] // 12
    freqs_cis = freqs_cis.repeat(segment_duration, 1)    
